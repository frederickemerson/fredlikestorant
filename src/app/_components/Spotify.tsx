...
const firstp:string =  (str: string) => {
...
const Spotify = async () => {
    ...
    if (response.ok && response.headers.get('Content-Type')?.includes('application/json')) {
      ...
      const name = limit(current.item.name)
      return(
        <Popover>
        ...
        <PopoverContent className="w-auto p-4 rounded-lg bg-gray-800 text-gray drop-shadow-lg border-black shadow-black  ">
          ...
          <div className="space-y-2"b-black>
             <div className="flex flex-row md:flex-row">
                ...
                <div className="flex flex-col m-2">
                    <h2 className="flex flex-wrap text-xl md:text-2xl lg:text-3xl font-bold">{firstp(current.item.name)}</h2>
                    <p className="text-gray-400 text-sm">Album : {current.item.album.name}</p>
                    <p className="text-gray-400 text-md">{current.item.artists[0].name}</p>
                </div>
             </div>
          </div>
        </PopoverContent>
